...
작업 분류용. 두 축으로 운영: `type`(작업 유형: 브로슈어·리플렛, 회사소개서, 패키지 등)과 `industry`(산업 분야: 뷰티·웰니스, 공공기관, 테크·플랫폼 등).

| 칸 | 타입 | 비고 |
|---|---|---|
| `id` | uuid (PK) | 자동 |
| `name` | text | 표시명 |
| `category` | text | `'type'` 또는 `'industry'` |
| `created_at` | timestamptz | 자동 |

`designs.tags`는 이 테이블의 `id`들을 배열로 보관. 한 작업에 여러 태그(여러 type, 여러 industry 모두 가능) 다중 선택.
...
| `email` | text | 필수 |
| `company` | text? | 옵션 |
| `work_types` | text[] | 선택한 type 태그 ID 배열 + "기타" 선택 시 `'other'` 마커 포함 |
| `work_other` | text? | "기타" 선택 시 자유 입력 |
...
- 두 종류 필터 (각각 칩/pill 형태, 다중 선택, "All" 버튼으로 초기화)
  - **Industry 필터** — `tags` 중 `category='industry'`
  - **Work Type 필터** — `tags` 중 `category='type'`
...
**필터링 로직**:
- 검색어가 있으면 제목 또는 클라이언트에 포함된 작업만
- Industry 필터가 선택된 경우, 작업의 태그 중 하나라도 선택된 industry에 포함되어야 함
- Work Type 필터도 동일
- 모든 조건 AND
...
- 행별 액션: 편집, 삭제
- 필터: Industry/Work Type 칩 (공개 페이지와 동일한 패턴)
- 검색: 제목/클라이언트
...
- **이미지 업로드 영역**:
  ...
  - 이미지 카드 드래그로 순서 변경
- **태그 다중 선택 UI**:
  - Industry 그룹과 Work Type 그룹을 시각적으로 분리해서 표시
  - 저장은 같은 `tags` 배열에 합쳐서
...
4. `designs` 테이블에 insert/update

#### 5.8.2 `/admin/products`

제품 목록 + 신규/편집 폼. designs와 유사하되 단순 버전.
...
- Industry 그룹과 Work Type 그룹 분리 표시
- 신규 추가 폼 (이름 + 카테고리 선택)
- 편집/삭제 액션
...
### Published 토글
모든 공개 콘텐츠(`designs`, `products`)는 `published` 플래그로 노출 제어. 등록은 해두고 나중에 공개하는 워크플로 지원.
...
- [ ] `/admin/tags`에서 type/industry 태그 등록·편집·삭제 가능
- [ ] `/admin/designs`에서 작업 신규 등록 (이미지 다중 업로드 + 별표 대표 지정 + 드래그 순서 + 태그 다중 선택) 가능
- [ ] `/admin/designs`에서 작업 편집·삭제·공개 토글 가능
