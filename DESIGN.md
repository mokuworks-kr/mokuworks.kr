...
- Design 상세 등의 풀블리드 이미지도 컨테이너 너비 안에서 폭 100%로 흐른다 (≤1760px). 4K 이상 모니터에서 무한히 펼쳐지지 않는 안전장치.
...
- **좌측은 sticky**: `lg:sticky lg:top-8 lg:self-start` — viewport 상단 32px에 고정되고, 우측 컨텐츠가 스크롤된다. `self-start`가 없으면 grid가 컬럼 높이를 늘려 sticky가 깨진다.
