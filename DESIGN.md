...
모든 페이지에 단일 최대 너비를 적용해 페이지 간 폭이 흔들리지 않게 한다.

```css
--container-page: 1536px;
```

규칙:
- Header, Footer, 모든 페이지의 최상위 섹션은 `max-width: var(--container-page)` + `margin-inline: auto`로 가운데 정렬한다.
- 좌우 패딩(컨테이너 내부): 모바일 `--space-4` (16px), 태블릿/데스크톱 `--space-8` (32px).
- 카드 그리드는 이 컨테이너 안에서 반응형 컬럼 수가 조정된다 (§7 참조).
- Design 상세의 풀블리드 이미지도 동일 컨테이너 안에서 폭 100%로 흐른다 (≤1536px). 4K 이상 모니터에서 무한히 펼쳐지지 않는 안전장치.

본문성 페이지(About/Contact/Home 등)는 추후 가독성 보정으로 본문 블록에 별도 max-width(예: `max-w-prose`)를 줄 수 있다. 이는 §5 컴포넌트 단계에서 결정한다.

Tailwind 클래스 사용: `max-w-page mx-auto` (Tailwind v4 `--container-*` 토큰 → 유틸리티 자동 생성).
