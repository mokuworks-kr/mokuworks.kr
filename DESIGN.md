...
`max-w-page` 안쪽에서 폼·상세 row는 한 단 더 좁힌다. FHD admin 세션에서 단일 컬럼 인풋이 1856px로 늘어나 타이핑·스캔이 부담스러워지는 문제를 해결한다.

규칙:
- 적용 대상: `/admin/products/{new,edit}`, `/admin/design/{new,edit}`, `/admin/tags`, `/admin/inquiries/[id]`. 단순 좌측 정렬(컨테이너 내부 `mx-auto` 사용 안 함).
- `/admin/login`은 `max-w-sm`(384px) 별도 — 인증 폼은 더 좁힘.
- 너비 = sm 분기점(640px). 분기점과 같은 숫자라 페이지 폭 ladder(640 → 768 → 1024 → 1280 → 1920)에서 자연스러운 한 칸을 차지한다.
- DesignForm의 이미지 보드 3-column 그리드는 폼 폭 안에서 ~205px/썸네일이 되어 살짝 작아지지만, admin 한정이라 클릭 가능성만 보장되면 OK.
- 저장 버튼: Primary 스타일 유지 + `self-end`(폼 우측 끝 정렬). 시선 흐름 자연스러움 + 버튼 너비가 텍스트 길이로만 결정되어 위치는 항상 우측 고정.
...
- **`top` 값 규칙 (중요)**: 좌측 컬럼의 자연 시작 위치(헤더 64px + 컨테이너 상단 패딩)와 정확히 일치시킨다. 그래야 첫 픽셀부터 sticky가 잠겨서 컬럼이 이동 없이 같은 자리에 머무르고, 헤더가 자연 흐름으로 위로 사라지는 동안에도 시각적 점프가 발생하지 않는다.
  - 페이지별 권장값:
    - `/design/[slug]` (article `py-12 md:py-16`): `lg:top-32` (128px = 64 + 64)
    - `/contact` (section `py-24 md:py-32`): `lg:top-48` (192px = 64 + 128)
...
호버: opacity 0.85
```
- 폼 컨테이너(`max-w-form`) 안에서 사용 시 정렬: `self-end`(우측 끝). 시선이 라벨/인풋 → 우측 하단 행동 버튼 순으로 흐른다.
- 적용: `/admin/{products,design}/{new,edit}` 저장, `/admin/inquiries/[id]` 메일로 답장.
- `/contact` 의뢰 보내기 버튼은 2-column 레이아웃 안에서 `self-start` 유지 — 폼 컬럼(`lg:col-span-2`)이 화면을 가득 채우지 않아 자연스러운 좌측 정렬이 더 적절.
...
**Date input (월 단위, `<input type="month">`)**
- DB가 `'YYYY-MM'` 텍스트만 보관하므로 일(day) 단위 datepicker는 무의미. 브라우저 네이티브 month picker(`<input type="month">`)를 그대로 사용한다.
- 출력 형식이 정확히 `YYYY-MM`이라 DB·SPEC와 호환. 의존성 0.
- 스타일은 Form input 토큰(`bg-cloud rounded-sm px-4 py-3` 등) 그대로 적용. OS별로 picker UI가 살짝 다르지만 admin 한정이라 OK.
